...
outputDir = f"History/Pretrained Models with Keras Tuner {whichCategory} {whichMagnification}"
...
# Start the hyperparameter search by fitting the tuner on the training data and validating on the validation data.
tuner.search(
  trainGen,
  epochs=maxEpochs,
  ...
  callbacks=[
    ...
    EarlyStopping(patience=0.1 * maxEpochs),
    # Reduce learning rate when a monitored metric plateaus.
    ReduceLROnPlateau(factor=0.5, patience=0.05 * maxEpochs),
  ],
  verbose=1,
)
